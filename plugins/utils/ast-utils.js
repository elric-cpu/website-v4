...
import { isJSXIdentifier, isJSXMemberExpression } from "@babel/types";
...
export function findJSXElementAtPosition(ast, line, column) {
  ...
  return (
    targetNodePath || (closestDistance <= threshold ? closestNodePath : null)
  );
}
...
export function extractCodeBlocks(filePath, line, column, domContext) {
  try {
    ...
    if (!targetNodePath) {
      return {
        success: false,
        error: "Target node not found at specified line/column",
        domContext,
      };
    }
    ...
    const specificLine = generateCode(
      targetNodePath.parentPath?.node || targetNodePath.node,
    );
    ...
    return {
      success: false,
      error: "Failed to extract code blocks",
      domContext,
    };
  }
}
