...
const SubcontractorCompleteProfile = () => {
  const { user } = useAuth();
  ...
  const handleSubmit = async (event) => {
    ...
    const sanitizedProfile = {
      ...
      license_number: sanitizeText(profile.license_number),
      insurance_expiry: profile.insurance_expiry || null,
    };

    setLoading(true);
    const { error } = await supabase.from("subcontractors").upsert(
      [
        {
          id: user.id,
          email: user.email,
          name: sanitizeText(user.user_metadata?.full_name || user.email),
          ...sanitizedProfile,
        },
      ],
      { onConflict: "id" },
    ...
    await supabase.auth.updateUser({
      data: { onboarded_subcontractor: true },
    });

    toast({
      title: "Profile complete",
      description: "Thanks! You can now access the portal.",
    });
    setLoading(false);
  ...
  return (
    <>
      <SEO
        ...
        description="Complete your subcontractor profile to access the portal."
      ...
      <main className="min-h-screen bg-[#FAF6EE] py-16 px-4">
        <div className="max-w-2xl mx-auto bg-white border border-[#D4C5A5] p-8">
          ...
          <p className="text-gray-700 mt-2">
            Finish your subcontractor profile to unlock full portal access.
          </p>
...
export default SubcontractorCompleteProfile;
